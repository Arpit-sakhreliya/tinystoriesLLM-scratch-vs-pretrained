"""
tinystories_scratch_training.py
-------------------
Builds a TinyStories-1M model from its architecture configuration after removing
all pretrained weights, preserving only the network architecture. Initializes
new random weights and trains the model entirely from scratch, requiring more
epochs since it must learn both language patterns and the target task.
"""
...
DATA_FILE    = "data.json"
SAVE_DIR     = "./tinystories_finetuned"
EPOCHS       = 200
BATCH_SIZE   = 8
LR           = 5e-4          # slightly higher start
MAX_SEQ_LEN  = 128
LOG_EVERY    = 100
LOSS_TARGET  = 0.08          # stop when avg loss hits this
...
def train(args):
    ...
    # Cosine annealing with warm restarts — avoids the "LR → 0 and stalls" problem
    # T_0=10 means it restarts every 10 epochs; after each restart loss can drop further
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if avg <= LOSS_TARGET:
            print(f"  >> Reached loss target {LOSS_TARGET} — stopping.")
            break
...
def infer(args):
    ...
    test_inputs = [
        "John Sarah visited on March 5 and paid $297.",
        "Brown Jack arrived on April 12 and paid $1250.",
        "Michael cat visited on January 8 and paid $70.",
        "Patrick Lee checked in on July 4 and paid $189.99.",
    ]
